...
def main() -> None:
    ...
    with httpx.Client(base_url="http://frontend", timeout=120) as client:
        ...
        check(
            "A01-admin-profile",
            admin_profile_response.status_code == 200
            and admin_profile.get("isAdmin") is True
            and admin_profile.get("companyName") == "Mayako",
            admin_profile,
        )
        ...
        check(
            ...
            and account_rows
            and all(row.get("companyName") == "Mayako" for row in account_rows),
            {
                ...
                "companies": sorted({row.get("companyName") for row in account_rows}),
            },
        )
        ...
        check(
            ...
            and updated_account.get("companyName") == "Mayako",
            {
    ...
    print(json.dumps({
        "metadata": {
            "suite": "Stock Check price permissions and PostgreSQL history",
