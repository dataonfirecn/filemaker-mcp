"""Run detailed live Q&A checks for Mayako products, parts, and orders.
...
def main() -> None:
    ...
    with httpx.Client(base_url="http://frontend", headers=headers, timeout=120) as client:
        ...
        runner.query(
            ...
            domain="出库单",
            title="精确追踪号",
            prompt="tracking 910038198088",
            expected_type="order",
            extra=lambda payload: [
                _check(
                    "追踪号精确命中",
                    payload.get("foundCount") == 1
                    and _value_at(payload, "rows", 0, "trackingNumber") == "910038198088",
                    (
        ...
        runner.query(
            ...
            extra=lambda payload: [
                _check(
                    "产品关联有命中",
                    isinstance(payload.get("foundCount"), int) and payload.get("foundCount") > 0,
                    f"found={payload.get('foundCount')}",
                )
            ],
        )
