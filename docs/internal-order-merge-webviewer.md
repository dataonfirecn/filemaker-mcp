...
如果目标布局使用不同的客户表 occurrence，只需替换上面的字段引用；URL 参数名不要改变。当前实际关联链为 `客戶::ID → 出貨單::customer_id → 出貨單資料::ID_出貨單`，其中后一个关联使用 `出貨單::id`。`customerId` 必须与 `出貨單::customer_id` 的实际存储值一致，否则只读列表仍可按客户名称显示，但 Web 写入会被归属校验拒绝。`ID_DB`、`ID_DB_Customer` 和 `ID_SYNC` 均属于外部系统对接字段，本功能不读取也不写入。
...
3. 在 `訂單 資料_業務_EDIT` 创建主记录，写入日期、`訂單型態 = 零件包` 和 `訂單分類 = 合併單`，让 FileMaker 自动生成订单 ID 和 `NB…` 内部订单编号；随后通过 `@出貨單` 写入 `customer_id`。`requestId` 的防重复状态保存在后端专用 `web_merge_request` 表，不占用 FileMaker 的任何对接字段。
4. 读取自动生成的 `id` 和 `internal_id`，整理本次合并追溯文字并写入新订单的 `log` 字段。日志包含时间、操作人、客户、目标 `NB…`、来源 `NB…`、来源/汇总明细数、每个产品的编号、名称和汇总数量，以及 `requestId`。
...
- `@出貨單`：`id`、`internal_id`、`customer_id`、`log`
...
- “全部”只展示具备 `internal_id` 和业务订单 ID、可交给合并脚本处理的记录；缺少内部单号的历史记录会在页脚单独计数，不会静默混入可选择列表
