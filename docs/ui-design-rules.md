...
- 简繁切换：文案只写简体（唯一来源），**不要**为繁体另写文案或硬编码繁体字。用户切到繁体后，`frontend/src/i18n/chineseVariant.ts` 只转换源码里写死的界面文字（标题、导航、按钮、标签、表头、提示）；后端 / FileMaker 读出的数据、输入框里的内容一律不转换。因此界面文案要写成字面量（含 `` `共 ${n} 条` `` 这类模板），数据不要写成字面量再显示；需要强制保持原样的元素加 `translate="no"`。
