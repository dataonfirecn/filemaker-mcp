...
class NightlyMaintenanceWorker:
    ...
    async def _run_query_analytics(self, now: datetime) -> str:
        ...
        failed_examples = list(summary["failedExamples"])
        zero_results = list(summary["zeroResultExamples"])
        error_count = sum(int(count) for _, count in summary["errors"])
        warning_count = sum(int(count) for _, count in summary["warnings"])
        exception_rows = [
            {
                "category": "查询失败" if item["status"] == "failed" else "需要澄清",
                "severity": "critical" if item["status"] == "failed" else "warning",
                "title": item["prompt"] or "未命名查询",
                "description": item["error"] or item["interpretedPrompt"],
                "impact": "该问题未能直接返回有效结果。",
                "suggestedAction": "检查字段语义、别名或补充用户查询条件。",
                "owner": "数据与AI运营",
            }
            for item in failed_examples[:10]
        ]
        report_status = (
            "warning" if failed_examples or error_count or warning_count else "success"
        )
        metrics = [
            ...
            _metric("analyzed", "本轮归一化", analyzed, sort_order=3),
            _metric("meaningful", "有效问法", meaningful, sort_order=4),
            _metric(
                "failed_or_clarification",
                "失败与澄清",
                len(failed_examples),
                severity="warning" if failed_examples else "info",
                sort_order=5,
            ),
            _metric(
                "zero_results",
                "零结果查询",
                len(zero_results),
                severity="warning" if zero_results else "info",
                sort_order=6,
            ),
        ]
        ...
        summary_text = (
            f"共处理 {summary['total']} 次查询，平均耗时 "
            f"{summary['averageDurationMs']} ms；发现 {len(failed_examples)} 个失败或澄清样例、"
            f"{len(zero_results)} 个零结果样例。"
        )
        published = await self.reports.publish(
...
def _query_quality_html(
    *,
    ...
    failed_rows = "".join(
        ...
        for item in summary["failedExamples"]
    ) or _html_empty_row(4)
    zero_rows = "".join(
        _html_row(
            (
                item["prompt"],
    ...
    body = f"""
      ...
      <section><h2>失败与澄清样例</h2><div class="table-wrap"><table>
        <thead><tr><th>状态</th><th>原问题</th><th>归一化</th><th>错误</th></tr></thead>
    ...
    return _html_document(
        title="DMS 当日查询质量与别名分析",
        report_date=report_date,
        status=status,
        summary=(
            f"查询总数：{summary['total']}；平均耗时：{summary['averageDurationMs']} ms；"
