...
def send_nightly_report_email(
    ...
    *,
    ...
    message["Subject"] = f"[{status_label}] {brand} 夜间报告 {report_date} · {title}"
    message["From"] = formataddr(
        (f"{brand} 夜间报告", settings.customer_smtp_from_email.strip())
    )
    ...
    message.set_content(
        "\n".join(
            ...
            for part in (
                ...
                f"状态：{status_label}",
                f"摘要：{summary}",
                f"报告中心：{report_url}" if report_url else "",
                "",
                f"本邮件由 {brand} 夜间任务自动发送。",
            )
            if part
        )
    )
    ...
    <tr><td align="center" style="padding:24px 12px">
      <table role="presentation" width="680" cellspacing="0" cellpadding="0" border="0"
        ...
        <tr><td style="padding:28px;background:#0f766e;color:#ffffff">
          <div style="font-size:11px;font-weight:700;letter-spacing:.08em;opacity:.8">{html.escape(brand.upper())} NIGHTLY REPORT · {report_date}</div>
        ...
        <tr><td style="padding:20px 28px 4px">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
            <tr>
              ...
              <td width="10"></td>
              <td style="border:1px solid #e2e9e9;border-radius:9px;padding:13px">
                <div style="color:#718087;font-size:10px">重要异常</div>
        ...
        <tr><td style="padding:18px 28px 4px">
          <h2 style="margin:0 0 10px;color:#173d40;font-size:15px">重要异常</h2>
        ...
        <tr><td style="border-top:1px solid #e8eeee;padding:16px 28px;color:#87949a;font-size:10px;line-height:1.5">
          本邮件由 {html.escape(brand)} 夜间任务自动发送。完整HTML报告和历史记录保存在服务器报告中心。
        </td></tr>
      </table>
    </td></tr>
  </table>
...
def _exception_rows(exceptions: Iterable[dict[str, Any]]) -> str:
    rows: list[str] = []
    for item in exceptions:
        ...
        detail = "<br>".join(
            part for part in (description, f"建议：{action}" if action else "") if part
        )
        rows.append(
            f"""<tr><td style="border-left:3px solid #b7791f;border-bottom:8px solid #ffffff;background:#fff8e6;padding:10px 12px">
              <strong style="color:#553b11;font-size:11px">{title}</strong>
              {f'<div style="margin-top:4px;color:#725f3a;font-size:10px;line-height:1.55">{detail}</div>' if detail else ''}
            </td></tr>"""
        )
    if not rows:
