"""Copy product container data into the standalone ProductAssets table.

Safety boundary:

* ``@products`` is a read-only source.
* Every create/update/container upload targets ``ProductAssets``.
* The default mode is a dry run. Writes require ``--commit``.

Examples:

    .venv/bin/python backend/scripts/migrate_product_assets.py --limit 50
    .venv/bin/python backend/scripts/migrate_product_assets.py --limit 3 --commit
"""
...
MIGRATION_ACTOR = "codex_product_asset_migration"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Maximum source products to scan; use 0 for every product (default: 50).",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Maximum concurrent target copies in commit mode (default: 4).",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
        help="Write records and containers to ProductAssets. Without this flag the run is read-only.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def _filename_from_response(url: str, response: httpx.Response) -> str:
    ...
    return filename or "product-asset.bin"
...
async def _validate_layouts(client: FileMakerClient) -> None:
    ...
    missing_source = sorted(spec.source_field for spec in IMAGE_SPECS if spec.source_field not in source_fields)
    if missing_source:
        raise RuntimeError(f"Source layout is missing image fields: {missing_source}")
...
async def _target_record_index(
    client: FileMakerClient,
...
async def _copy_asset(
    ...
    *,
    ...
    target_data = {
        "product_sku_fk": _text(source_fields.get("product_sku")),
        "system_product_number_fk": _text(source_fields.get("系統產品編號")),
        "id_client_snapshot": _text(source_fields.get("id_client")),
        "asset_type": spec.asset_type,
        "visibility": spec.visibility,
        "legacy_source_field": spec.source_field,
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            async for record in _source_records(
                ...
                summary["sourceProductsScanned"] += 1
                ...
                if args.commit and summary["sourceProductsScanned"] % 250 == 0:
                    print(
                        "progress "
                        + json.dumps(
                            {
