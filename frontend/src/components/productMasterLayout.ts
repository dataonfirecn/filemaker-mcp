...
export const fieldLabels: Record<string, string> = { 類別: '类别', 車子比例: '车子比例', 車款: '车型', 產品分類: '产品分类', Brand_ID: '品牌编号', Category: '目录分类', category1: '一级分类编号', category2: '二级分类编号', category3: '三级分类编号', ShowStock: '显示库存', 產品位置: '产品位置', 位置: '位置', 有現貨: '库存状态', 追加訂購的等級: '追加订购等级', lastOrderElapsedDays: '距上次下单天数', Upload_Date: '上架日期', 重量: '重量', 尺寸: '尺寸', 'N.W淨重': '净重', 'G.W毛重': '毛重', 產品平均CBM: '平均体积 · CBM', product_sku: 'SKU', product_name: '英文名称', 產品名稱_中文: '中文名称', 系統產品編號: '系统编号', 審核: '审核状态', MOQ: '最小起订量 · MOQ', opencost: '原始成本备注', RMB成本: 'RMB成本', 美金成本: '美金成本 · USD', 'EX-Price': 'EX-Price · 出厂价', 台幣出廠: '台币出厂价 · NTD', RMB出廠: 'RMB出厂价 · CNY', Retail_Price_USD: '零售价格 · USD', 組裝成本USD: '组装成本 · USD', 提成: 'A 积分', 提成B: 'B 积分', Client: '客户', id_client: '客户编号', created_at: '建立日期', created_by: '建档人', updated_at: '修改日期', updated_by: '修改人', Product_intro_english: '英文产品介绍', G_W: '毛重', stock: '产品库存', Stock_USD: '库存金额 · USD', PrePaid_stock_USD: '预付库存 · USD', 新產品: '新产品', 加入目錄: '加入目录', 條形碼: '条形码', 客戶SKU: '客户 SKU', 法文名稱: '法文名称', 描述: '描述', Remarks: '备注', 附註: '附注', 建議報價: '建议报价', 報價積數: '报价积数', 成本加成: '成本加成', 匯率: '汇率', 組裝成本: '组装成本', 其他成本估價: '其他成本估价', '彩盒尺寸 長': '彩盒长', '彩盒尺寸 寬': '彩盒宽', '彩盒尺寸 高': '彩盒高', 彩盒尺寸CBM: '彩盒 CBM', '外箱尺寸 長': '外箱长', '外箱尺寸 寬': '外箱宽', '外箱尺寸 高': '外箱高', 外箱尺寸CBM: '外箱 CBM', '車款 車型 A': '适配车型 A', '車款 車型 B': '适配车型 B', '車款 車型 C': '适配车型 C', '車款 車型 D': '适配车型 D', 時薪: '时薪', 準備工時數量: '准备计时数量', 準備工時分: '准备工时 · 分', 準備工時秒: '准备工时 · 秒', 包裝工時分: '包装工时 · 分', 包裝工時秒: '包装工时 · 秒', 準備工時單包: '准备工时 · 秒/包', 包裝工時單包: '包装工时 · 秒/包', 準備包裝工錢: '准备工钱', 包裝工錢: '包装工钱', 包裝總工錢: '包装总工钱', 總人工成本: '总人工成本' };
export const sectionFields: Record<string, string[]> = {
  ...
  '报价参考': [...priceFields, '下單數量', '訂單數量', '彩盒尺寸 長', '彩盒尺寸 寬', '彩盒尺寸 高', '彩盒尺寸CBM', '外箱尺寸 長', '外箱尺寸 寬', '外箱尺寸 高', '外箱尺寸CBM', 'N.W淨重', 'G.W毛重', '產品平均CBM'],
  '產品BOM': ['BOM計數', '關聯編號'],
  '產品BOM排序': ['排序選取'],
...
/** 规格书按顺序添加；包装与标签各位置有固定名称，页面按这里的顺序列出。 */
export const specFieldOrder = ['產品規格書', '產品規格書2'];
export const packagingFieldOrder = ['彩盒', '外紙箱', '貼紙', '標籤檔案', '標籤檔案後', '發料標籤檔案'];
export function assetGroup(name: string): string {
  if (productPhotoFields.includes(name)) return '產品照片';
  if (specFieldOrder.includes(name)) return '产品规格书';
  if (/^銷售紀錄照片/.test(name)) return '銷售紀錄';
  if (packagingFieldOrder.includes(name)) return '包装与标签';
  return '產品附件';
}
...
export const basicSections: Record<string, string[]> = {
  ...
  '报价信息': ['RMB成本', '美金成本', 'EX-Price', '台幣出廠', 'RMB出廠', 'MOQ', 'Retail_Price_USD', '建議報價', 'opencost', '組裝成本USD', '組裝成本', '其他成本估價', '匯率', '成本加成', '報價積數', '提成', '提成B', '時薪', '準備工時分', '準備工時秒', '準備工時數量', '包裝工時分', '包裝工時秒', '準備工時單包', '包裝工時單包', '準備包裝工錢', '包裝工錢', '包裝總工錢', '總人工成本'],
...
export const priceBands: Array<{ title: string; fields: string[] }> = [
  { title: '成本', fields: ['RMB成本', '組裝成本', '其他成本估價', '美金成本', '組裝成本USD'] },
  { title: '售价', fields: ['RMB出廠', '台幣出廠', 'Retail_Price_USD', '建議報價', 'EX-Price', 'MOQ', '報價積數'] },
  { title: '人工成本', fields: ['時薪', '準備工時分', '準備工時秒', '準備工時數量', '包裝工時分', '包裝工時秒', '準備工時單包', '包裝工時單包', '準備包裝工錢', '包裝工錢', '包裝總工錢', '總人工成本'] },
...
/**
 * 本表内由其他字段算出来的字段，页面按原公式实时重算（公式来源：docs/product-quotation-field-audit-20260924.md）。
 * 返回值：字符串 = 页面显示值（'' 表示输入还不全；'?' 表示公式异常，如分母为 0）；
 * undefined = 依赖的数据取不到（BOM 成本、库存），沿用库里保存的值。
 * 这些字段只用于显示，不进入保存内容。
 */
export const derivedFieldNames = ['彩盒尺寸CBM', '外箱尺寸CBM', '產品平均CBM', '建議報價', '有現貨', 'Stock_USD', '準備工時單包', '包裝工時單包', '準備包裝工錢', '包裝工錢', '包裝總工錢', '總人工成本'] as const;
type Num = number | null | undefined;   // null：输入为空；undefined：外部数据取不到
const toNumber = (raw: string): number | null => { const text = raw.trim().replace(/,/g, ''); if (!text) return null; const n = Number(text); return Number.isFinite(n) ? n : null; };
const formatNumber = (n: number) => Number.isFinite(n) ? String(Number(n.toFixed(6))) : '?';

export function derivedValue(name: string, raw: (name: string) => string): string | undefined {
  const cache = new Map<string, Num>();
  const input = (n: string): number | null => toNumber(raw(n));
  const zero = (n: string): number => input(n) ?? 0;
  const volume = (prefix: string): Num => { const parts = [' 長', ' 寬', ' 高'].map(x => input(prefix + x)); return parts.some(x => x === null) ? null : (parts[0]! * parts[1]! * parts[2]!) / 1000000; };
  const calc = (n: string): Num => {
    if (cache.has(n)) return cache.get(n);
    let out: Num;
    switch (n) {
      case '彩盒尺寸CBM': case '產品平均CBM': out = volume('彩盒尺寸'); break;
      case '外箱尺寸CBM': out = volume('外箱尺寸'); break;
      case '建議報價': { const usd = input('美金成本'); out = usd === null ? undefined : usd / ((100 - zero('報價積數')) / 100); break; }
      case 'Stock_USD': { const stock = input('stock'), price = input('EX-Price'); out = stock === null || price === null ? undefined : stock * price; break; }
      case '準備工時單包': case '包裝工時單包': {
        const p = n === '準備工時單包' ? '準備' : '包裝';
        out = input(p + '工時分') === null && input(p + '工時秒') === null ? null : (() => { const qty = input('準備工時數量'); return !qty ? NaN : (zero(p + '工時分') * 60 + zero(p + '工時秒')) / qty; })();
        break;
      }
      case '準備包裝工錢': case '包裝工錢': {
        const per = calc(n === '準備包裝工錢' ? '準備工時單包' : '包裝工時單包'), wage = input('時薪');
        out = per == null || wage === null ? null : (wage / 60 / 60) * per; break;
      }
      case '包裝總工錢': { const a = calc('包裝工錢'), b = calc('準備包裝工錢'); out = a == null && b == null ? null : (a ?? 0) + (b ?? 0); break; }
      case '總人工成本': { const labor = calc('包裝總工錢'), assembly = input('組裝成本'); out = labor == null && assembly === null ? null : (assembly ?? 0) + (labor ?? 0); break; }
      default: out = input(n);
    }
    cache.set(n, out); return out;
  };
  if (!(derivedFieldNames as readonly string[]).includes(name)) return undefined;
  if (name === '有現貨') { const stock = input('stock'); return stock === null ? undefined : stock > zero('追加訂購的等級') ? '有現貨' : '追加訂購'; }
  const result = calc(name);
  return result === undefined ? undefined : result === null ? '' : formatNumber(result);
}
